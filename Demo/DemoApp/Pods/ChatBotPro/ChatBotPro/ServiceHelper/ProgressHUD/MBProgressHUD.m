...
//                                                                     (CGFloat) ((view.frame.size.height - 20) * 0.5), 55, 20)];
   // hud.hudColor = getColor(78.0, 188.0, 28.0, 1.0);29, 83, 121
    hud.hudColor = getColor(29.0, 83.0, 121.0, 1.0);
...
+ (id)showToastHUDAddedTo:(UIView *)view animated:(BOOL)animated {
    MBProgressHUD *hud = [[self alloc] initWithView:view];
    [view addSubview:hud];
    [hud show:animated];
    return hud;
...
+ (id)HUDForView:(UIView *)view {
	...
	for (UIView *subview in subviewsEnum) {
		if ([subview isKindOfClass:[ZTTumblrHud class]]) {
			return (ZTTumblrHud *)subview;
		}
	}
	return nil;
}
